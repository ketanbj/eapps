...
public abstract class ActivityManagerNative extends Binder implements IActivityManager
{
	...
	case START_ACTIVITY_EPHEMERAL_TRANSACTION:
        {
            data.enforceInterface(IActivityManager.descriptor);
            IBinder b = data.readStrongBinder();
            Intent intent = Intent.CREATOR.createFromParcel(data);
	    ApplicationInfo ainfo = ApplicationInfo.CREATOR.createFromParcel(data);
            String pname = data.readString();
            Bundle options = data.readInt() != 0
                    ? Bundle.CREATOR.createFromParcel(data) : null;
            int result = startActivityEphemeral(pname, intent,ainfo, options);
            reply.writeNoException();
            reply.writeInt(result);
            return true;
        }
...
class ActivityManagerProxy implements IActivityManager
{
	...
	public int startActivityEphemeral(String pname,Intent intent, ApplicationInfo ainfo, Bundle options) throws RemoteException {
	Parcel data = Parcel.obtain();
	Parcel reply = Parcel.obtain();
	intent.writeToParcel(data, 0);
	...
	mRemote.transact(START_ACTIVITY_EPHEMERAL_TRANSACTION, data, reply, 0);
	reply.readException();
	int result = reply.readInt();
	reply.recycle();
	data.recycle();
