...
public final class ActivityStackSupervisor implements DisplayListener {
	...
	// FIXME: Create process record for eapp here
    ...
    final int startActivityEphemeral(String pname,Intent intent, ApplicationInfo ainfo, Bundle options) {

        // FIXME: This name shuld be app name
	ProcessRecord pr = null;
	Slog.w(TAG, ">>> ActivityStackSupervisor.startActivityEphemeral()");
	pr = mService.startProcessLocked(pname, ainfo, true, 0,"activity", intent.getComponent(), false, false, true);
	Slog.w(TAG, "<<< ActivityStackSupervisor.startActivityEphemeral()");
	if(pr != null)
       		return ActivityManager.START_SUCCESS;
	else
