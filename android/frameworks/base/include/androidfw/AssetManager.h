...
public:
    ...
    /* Required for checking whether this app is ephemeral app or Not*/
    bool readEphemeralApps();
    Vector<String8> eapps;
    ...
    void getAppName(const char*, char**);

    int eapp_getAsset(char*, const char*, unsigned char**, int*);

    int check_folder(char* appName, const char* assetName);
    void _mkdir(const char *dir);
    int mkpath(const char* path, mode_t mode);
    int __mkpath(char *path);
