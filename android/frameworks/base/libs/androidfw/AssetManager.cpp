...
// 
// Type make libandroidfw -jX to build .so file
//
...
static const char *data_tail = "-1/base.apk";
static const char *dot_apk = ".apk";
static const char *data_eapp = "/data/ephemeralapps/";
//static const char *data_eapp = "/data/media/";
#endif
      ...
      mSocketfd(-1)
{
   ...
   bool res = readEphemeralApps();
   if(!res){
	ALOGV(">>> AssetManager::AssetManager Couldn't read ephemeral apps\n");
   }
}
...
AssetManager::~AssetManager(void)
{
   ...
   if(mSocketfd > 0) CloseSocket(mSocketfd);
}

bool AssetManager::readEphemeralApps(void)
{
     FILE* fp = fopen("/data/ephemeralapps/app-list", "r");
     if (fp == NULL) {
        ALOGV("Unable to open strapp config");
         return false;
     }
	...
	eapps.add(String8(cur_line));

     }

     ALOGV("Added %d Ephemeral apps from app-list", eapps.size());
     return true;
}
...
bool AssetManager::addAssetPath(const String8& path, int32_t* cookie)
{
    ...
    ALOGV("[%s] In %p Asset %s path: %s", 
        __FUNCTION__, this, ap.type == kFileTypeDirectory ? "dir" : "zip", ap.path.string());
...
/************************ JOSEPH ************************
 *  open() and openNonAsset() should be modified to 
 *  stream the assets from ebox. 
 *******************************************************/
...
Asset* AssetManager::open(const char* fileName, AccessMode mode)
{
    ...
    while (index > 0) {
        index--;
        unsigned char* nwBytes = NULL;
        char *appName = NULL;
        unsigned int nwLength = 0; 
        //FileMap* map;
        Asset* eAppAsset = NULL;
        int fd = -1;

     // map = new FileMap;
                // if ((fd = map->createBuffer((void**) nwBytes, nwLength)) < 0) {
                    // ALOGE(">>> [eapp-open] unable to make fd");
                    // map->release();
                // } else {
                // }
        getAppName(mAssetPaths.itemAt(index).path.string(), &appName);

        if(appName != NULL) {
            ALOGD(">>> [eapp-open] asset %s is requested in %s", assetName.string(), appName);
            if (!OpenSocket()) {
                ALOGE(">>> [eapp-open] unable to open socket");
            } else {
                /*
                * Gets the pointer to the data streamed from eBox 
                * Use Asset and FileMap to create mmaped Asset object and pass
                * to Android AssetManager to handle
                */
                nwLength = eapp_getAsset(appName, assetName.string(), &nwBytes, &fd);
				if(nwLength > 0 && fd > 0){
                	eAppAsset = Asset::createFromFD(fd, assetName.string(), mode);
                    if (eAppAsset == NULL) {
                       	ALOGE(">>> [eapp-open] unable to make Asset object :(");
                    } 
					else {
                       	ALOGD(">>> [eapp-open] created Asset object in %p !", eAppAsset);
    	            	free(appName);
						return eAppAsset;
                    }
                }
...
Asset* AssetManager::openNonAsset(const char* fileName, AccessMode mode, int32_t* outCookie)
{
    ...
    while (index > 0) {
        ...
        getAppName(mAssetPaths.itemAt(index).path.string(), &appName);
        if(appName != NULL) {
            ALOGD(">>> [eapp-openNonAsset] asset %s is requested in %s",fileName, appName);
            if (!OpenSocket()) {
                ALOGE(">>> [eapp-openNonAsset] unable to open socket");
            } else {
                nwLength = eapp_getAsset(appName, fileName, &nwBytes, &fd);
                if(nwLength > 0 && fd > 0){
					eAppAsset = Asset::createFromFD(fd, fileName, mode);
                	if (eAppAsset == NULL) {
                        ALOGE(">>> [eapp-open] unable to make Asset object :(");
                 	} else {
                      	free(appName);
...
Asset* AssetManager::openNonAsset(const int32_t cookie, const char* fileName, AccessMode mode)
{
    ...
    if (which < mAssetPaths.size()) {
        ...
        getAppName(mAssetPaths.itemAt(which).path.string(), &appName);
        if(appName != NULL) {
            ALOGD(">>> [eapp-openNonAsset] asset %s is requested in %s",fileName, appName);
            if (!OpenSocket()) 
                ALOGE(">>> [eapp-openNonAsset] unable to open socket");
    ...
    const ZipEntryRO entry, AccessMode mode, const String8& entryName)
{
    ...
    // ALOGV("[%s] USING Zip '%s'\n", __FUNCTION__, pEntry->getFileName());
    ...
    // dataMap->getFileName() --> AppName
    // entryName.string() --> asset name
    #ifdef EAPP
        char* appName;
        unsigned char* nwBytes = 0;
        unsigned int nwLength = 0; 
        int fd = -1;
        Asset* eAppAsset = NULL;

        getAppName(dataMap->getFileName(), &appName);
        if(appName != NULL) {
            ALOGD(">>> [eapp-openAssetFromZipLocked] asset %s is requested in %s", entryName.string(), appName);
            if (!OpenSocket()) {
                ALOGE(">>> [eapp-openAssetFromZipLocked] unable to open socket");
            } else {
                nwLength = eapp_getAsset(appName, entryName.string(), &nwBytes, &fd);
                if(nwLength > 0 && fd > 0){
                	eAppAsset = Asset::createFromFD(fd, entryName.string(), mode);
                	if (eAppAsset == NULL) {
                       	ALOGE(">>> [eapp-open] unable to make Asset object :(");
                    } 
					else {
...
/*
 *  Converts passed application name formed /data/PACKAGE-1/base.apk into PACKAGE.apk
 *
 */
void AssetManager::getAppName(const char* sourceName, char** result) {

	// FIXME: check for memory leaks
	char *pch = NULL,*last = NULL, *secondlast = NULL, *reqName = NULL;
	...
	*result = NULL;
	...
	// check if this is an ephemeral app
	if(eapps.empty())
        {
		bool check = readEphemeralApps();
		if(!check)
		{
                	ALOGV("No available ephemeral apps");
			*result = NULL;
			return;
		}
        }
	...
	String8 s8aN = String8(reqName);

	for (size_t i=0; i<eapps.size(); i++)
	{
		if(eapps[i] == s8aN)
		{
			ALOGV("Found ephemeral app %s", reqName);
			iseApp = true;
			break;
		}
	}
	
	if(iseApp) *result = strdupNew(pch);

	if(secondlast != NULL) delete[] secondlast;
	if(last !=NULL) delete[] last;
        if(pkgName != NULL) delete[] pkgName;
	return;
}

/*
 *  Reads generated ip-list from either file or global var.
 *  Creates socket fd if not created previously.
 *  socket_network_client function makes TCP connect call so that 
 *  application with connected socket fd can simply make 'write' call. 
 *
 *  FIXME - change file to global var
 */
bool AssetManager::OpenSocket() {
    if (mSocketfd > 0)
        return true;

    FILE* fp = fopen("/data/ephemeralapps/ip-list", "r");
    if (fp == NULL) {
        ALOGE("Unable to open strapp-ip config file");
        return false;
    }
    ...
    while(fgets(fLine,500,fp) != NULL) {
        ...
        mSocketfd = socket_network_client(curLine, 3000, SOCK_STREAM);
        if(mSocketfd > 0) {
            ALOGD("Using strapp IP: %s", curLine);
            return true;
        }
    }
    return false;
...
bool AssetManager::CloseSocket(int sockfd)
{
	// FIXME: Expose this so that socket gets clean up properly
	// socket_network_cleanup(sockfd);
	return true;
}

/* 
 *  Calls ephemeral_get to the designated data pointer of asset in app name. 
 *  Note that *buff contains data from eBox.
 */
int AssetManager::eapp_getAsset(char* appName, const char* assetName, unsigned char** buff, int* fd) {
    char* req = NULL, *path = NULL;
    int ret = -1, mFd = -1;
    FILE *file = NULL;
    size_t written = 0;
    *fd = -1;

    req = (char*) malloc(strlen(appName) + strlen(assetName) + 2);
    strcpy(req, appName);
    strcat(req, "/");
    strcat(req, assetName);
    strcat(req, "}");
    ALOGD(">>> [eapp-getAsset] req %s", req);

    ephemeral_req_type syt = STATIC_ASSET;

    ret = ephemeral_get(mSocketfd, syt, req, (void**) buff);
    if(ret > 0)
    {	
    	check_folder(appName, assetName);

    	path = (char*) malloc(strlen(data_eapp) + strlen(appName) + strlen(assetName) + 1);
    	strcpy(path, data_eapp);
    	strcat(path, appName);
    	strcat(path, "/");

    	if (strstr(assetName, "/") != NULL) {
        	char *repl_assetName = repl_str(assetName, "/", "$");
        	strcat(path, repl_assetName);
    	} else {
        	strcat(path, assetName);
    	}

    	file = fopen (path, "w");
    	if (file == NULL) {
        	ALOGE(">>> [eapp-createBuffer] uanble to fopen() - %s - %s", path,strerror(errno));
        	return -1;
    	}

    	written = fwrite(*buff, sizeof(char), ret, file);
    	if (written < ret) {
...
int AssetManager::check_folder(char* appName, const char* assetName) {
    char* path, *regex;//, *assetOnly, *dirOnly;
    char assetOnly[256], dirOnly[256], dirPath[345];
    struct stat st = {0};
    int regex_len;
    bool check_stat = false;

    path = (char*) malloc(strlen(data_eapp) + strlen(appName));
    strcpy(path, data_eapp);
    strcat(path, appName);
   ...
   /* if (strstr(assetName, "/") == NULL)
        ...
        regex = strrchr((char*)assetName, '/');
        bzero(assetOnly, 256);
        strncpy(assetOnly, (const char*) (regex + 1), strlen(regex+1));
        ALOGD(">>> [eapp-check_folder] assetOnly: %s", assetOnly);

        bzero(dirOnly, 256);
        strncpy(dirOnly, (const char*) (assetName), strlen(assetName) - strlen(regex));
        ALOGD(">>> [eapp-check_folder] dirOnly: %s", dirOnly);

        bzero(dirPath, 345);
        strcpy(dirPath, data_eapp);
        strcat(dirPath, appName);
        strcat(dirPath, "/");
        strcat(dirPath, dirOnly);
        ALOGD(">>> [eapp-check_folder] creating %s", dirPath);

        if (strstr(dirOnly, "/") == NULL) {
            if (stat(dirPath, &st) == -1) {
                mode_t process_mask = umask(0);
                mkdir(dirPath, S_IRWXU | S_IRWXG | S_IRWXO);
                umask(process_mask);
            }
        } else {// if (strstr(dirOnly, "/") != NULL) { // has sub-directory
            char rootDir[345], dirContent[345], dir_parent[345], dir_child[345];
            bzero(rootDir, 345);
            strcpy(rootDir, data_eapp);
            strcat(rootDir, appName);
            strcat(rootDir, "/");

            bzero(dirContent, 345);
            strcpy(dirContent, dirOnly);
            // char *dir_parent, *dir_child, *dir_copy;
        
            while (!check_stat && (strcmp(rootDir, dirPath) != 0)) {
                char *slash = strchr(dirContent, '/');            
                // dir_parent = (char*) malloc(strlen(dirContent) - strlen(slash));
                bzero(dir_parent, 345); //strlen(dirContent) - strlen(slash));
                strncpy(dir_parent, dirContent, strlen(dirContent) - strlen(slash));
                strcat(rootDir, dir_parent);
                strcat(rootDir, "/");

                // dir_child = (char*) malloc(strlen(slash) - 1);
                bzero(dir_child, 345); //strlen(slash) - 1);
                strcpy(dir_child, (const char*) (slash + 1));

                bzero(dirContent, 345);
                strcpy(dirContent, dir_child);

                ALOGD("creating %s and remaining %s\n", rootDir, dirContent);

                if (stat(rootDir, &st) == -1) {
                    mode_t process_mask = umask(0);
                    mkdir(rootDir, S_IRWXU | S_IRWXG | S_IRWXO);
                    umask(process_mask);
                    // mkdir(rootDir, 0777);
                }

                if (strchr(dirContent, '/') == NULL) {
                    check_stat = true;
                    strcat(rootDir, dirContent);
                }
            }
            ALOGD(">>> final creating %s\n", rootDir);
            mode_t process_mask = umask(0);
            int result = mkdir(rootDir, S_IRWXU | S_IRWXG | S_IRWXO);
            umask(process_mask);
            if (result < 0) {
                ALOGE(">>> error in mkdir(%s) [%s]", rootDir, strerror(errno));
            }
        }
        ...
        // lsocket = android_get_control_socket("installd");
        // if (lsocket < 0) {
        //     ALOGE("Failed to get socket from environment: %s\n", strerror(errno));
        // }

        // if (connect(lsocket, &addr, sizeof(addr)) < 0) {
        //     ALOGE("Connect on socket failed: %s\n", strerror(errno));
        // }
        // fcntl(lsocket, F_SETFD, FD_CLOEXEC);

        // int len = strlen(buff);

        // write(lsocket, &len, sizeof len);
        // write(lsocket, buff, len);
    } */
    return 1;
}

int AssetManager::__mkpath(char *path)
{
    struct stat sb;
    char *slash;
    int done = 0;
    ...
    while (!done) {
        ...
        *slash = '\0';
        ...
        if (stat(path, &sb)) {
            ...
            ALOGE("%s: %s", path, strerror(ENOTDIR));
            return (-1);
        }
...
int AssetManager::mkpath(const char* path, mode_t mode)
{
  char p[MAXPATHLEN];
  int c = 0, ret;
  char *lastslash = 0;
  struct stat s;
    ALOGD(">>> [eapp-check_folder] path %s", path);
  ...
  ALOGE(">>> [eapp-check_folder] impossible %s", path);

  assert("impossible state");
  return 0;
}

void AssetManager::_mkdir(const char *dir) {
    char tmp[512];
    char *p = NULL;
    size_t len;
    ALOGD("Creating %s", dir);
