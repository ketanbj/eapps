...
static jlong DexFile_openDexFileNative(JNIEnv* env, jclass, jstring javaSourceName, jstring javaOutputName, jint) {
  ...
  std::vector<std::string> error_msgs;
   ...
   bool success = false;
   //FIXME Try ro load from ephemeral app server
   EphemeralLinker* elinker = Runtime::Current()->GetEphemeralLinker();
   success =  elinker->OpenEphemeralDexFiles(sourceName.c_str(), outputName.c_str(), &error_msgs,dex_files.get());
  if(success && dex_files->size() != 0)
  {
    VLOG(ephemeral_linker) << "Using ephemeral classes.dex";
    return static_cast<jlong>(reinterpret_cast<uintptr_t>(dex_files.release()));	
  }  
  
  // Else try the regular path
  success = linker->OpenDexFilesFromOat(sourceName.c_str(), outputName.c_str(), &error_msgs,dex_files.get()); 
